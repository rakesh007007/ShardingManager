...
public class SessionFactoryFactory {
    ...
    public SessionFactory build(Environment environment,
                                DataSourceFactory dbConfig, List<Class<?>> entities) {
        return build(environment, dbConfig, entities, DEFAULT_NAME);
    }

    public SessionFactory build(Environment environment,
                                DataSourceFactory dbConfig, List<Class<?>> entities, String name) {
        final ManagedDataSource dataSource = dbConfig.build(
                environment.metrics(), name);
        return build(environment, dbConfig, dataSource, entities);
    }

    public SessionFactory build(Environment environment,
                                DataSourceFactory dbConfig, ManagedDataSource dataSource,
                                List<Class<?>> entities) {
        final ConnectionProvider provider = buildConnectionProvider(dataSource,
                dbConfig.getProperties());
        final SessionFactory factory = buildSessionFactory(dbConfig, provider,
                dbConfig.getProperties(), entities);
        final SessionFactoryManager managedFactory = new SessionFactoryManager(
                factory, dataSource);
    ...
    private SessionFactory buildSessionFactory(DataSourceFactory dbConfig,
                                               ...
                                               Map<String, String> properties, List<Class<?>> entities) {
        ...
        configuration.setProperty("jadira.usertype.autoRegisterUserTypes",
                "true");
